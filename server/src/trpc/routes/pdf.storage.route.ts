import { pdfStorageService, t } from '../../container.js';
...
export const pdfStorageRouter = t.router({
  uploadFile: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const { path, savedFileName } = await pdfStorageService.upload(
        file,
        filename,
        folderId,
      );
  ...
  getDiskUsage: protectedProcedure.query(async () => {
    return pdfStorageService.getDiskUsage();
  }),

  deleteFileGlobal: protectedProcedure
    .input(fileDeleteParamsSchema)
    .mutation(async ({ input }) => {
      await pdfStorageService.delete(input.filename);
      return { ok: true };
    }),
});
