...
export const LogCard = ({ log }: LogCardProps) => {
  return (
    <div className="border-(--border) border-b py-1 hover:bg-(--bg-hover)">
      ...
      <span className="text-(--meta-text)">[{log.source}]</span>{' '}
      <span>{log.msg}</span>
      {Object.keys(log).length > 4 && (
        <pre className="mt-1 max-w-full overflow-x-auto whitespace-pre-wrap break-all text-(--meta-text) text-xs">
          {JSON.stringify(
            Object.fromEntries(
